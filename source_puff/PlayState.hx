	...
	override public function create()
	{
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'nerd-rage':
			{
					defaultCamZoom = 0.8;
					curStage = 'downtown';
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('puffpuff/bg'));
					bg.scrollFactor.set(0.3, 0.3);
					bg.antialiasing = true;
					bg.active = false;
					add(bg);

					var signs:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.image('puffpuff/signs'));
					signs.setGraphicSize(Std.int(signs.width * 0.9));
					...
					var cars:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('puffpuff/prop_cars'));
					cars.setGraphicSize(Std.int(cars.width * 1.1));
